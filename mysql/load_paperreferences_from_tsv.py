...
def add_multiple_records_paperrefsinno(items, tablename):
    start = timer()
...
def add_records(items, tablename, last_added, i, num_added):
    ...
    add_multiple_records_paperrefsinno(items, tablename)
    num_added += len(items)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="load paper references from tsv")
    parser.add_argument("input", help="TSV filename")
    parser.add_argument("--sep", default='\t', help="delimiter to separate fields")
    parser.add_argument("--tablename", default='PaperReferences_inno', help="database table name to load into")
